...
socketio = SocketIO(
    ...
    manage_session        = True,
    path                  = '/socket.io',
    ping_timeout          = 60000,   # FIX: увеличено с 20s→60s для Render Free (wake latency)
    ping_interval         = 25000,   # FIX: увеличено с 10s→25s, меньше нагрузки на сервер
...
with app.app_context():
    try:
        ...
        try:
            ...
            '''))
            db.session.execute(text(
                'CREATE INDEX IF NOT EXISTS ix_telegram_auth_phone ON telegram_auth(phone)'
            ))
            db_safe_commit()
            print('✅ telegram_auth table ready')
        except Exception as _tg_e:
            db.session.rollback()
            print(f'telegram_auth table skip: {_tg_e}')
...
class TelegramAuth(db.Model):
    """Связка номер телефона ↔ Telegram chat_id для верификации."""
...
#  Пользователь пишет боту /start → получает код → вводит на сайте
# ══════════════════════════════════════════════════════════

TG_BOT_TOKEN   = os.environ.get('TG_BOT_TOKEN', '').strip()
TG_BOT_USERNAME = os.environ.get('TG_BOT_USERNAME', 'WayChatVerifyBot').strip()

def _gen_code():
    return str(random.randint(100000, 999999))

def normalize_phone(phone: str) -> str:
    """Приводит номер к стандартному виду: только цифры, 7XXXXXXXXXX для РФ."""
    import re as _re
    if not phone:
        return ''
    has_plus = phone.strip().startswith('+')
    d = _re.sub(r'\D', '', phone.strip())
    if not d:
        return ''
...
def _tg_send_code(phone: str, code: str) -> bool:
    """Ищем telegram_chat_id по номеру телефона и отправляем код.
    Пользователь должен заранее написать боту /start с номером телефона.
    """
    import traceback as _tb
    if not TG_BOT_TOKEN:
        app.logger.error('[TG] TG_BOT_TOKEN не задан в переменных окружения!')
        return False

    phone_norm = normalize_phone(phone)

    # Ищем chat_id в БД (сохранён когда пользователь писал боту)
    tg = TelegramAuth.query.filter_by(phone=phone_norm).first()
    if not tg:
        app.logger.warning(f'[TG] Нет chat_id для номера {phone_norm} — пользователь не писал боту')
        return False

    msg = (
        f'🔐 *Ваш код WayChat:* `{code}`\n\n'
        f'Введите его на сайте. Код действует 10 минут.\n'
    ...
    try:
        ...
        data = resp.json()
        if data.get('ok'):
            app.logger.info(f'[TG] ✅ Код отправлен на chat_id={tg.chat_id} ({phone_norm})')
            return True
        app.logger.error(f'[TG] ❌ Ошибка Telegram: {data}')
        return False
    except Exception as e:
        app.logger.error(f'[TG] Исключение: {e}\n{_tb.format_exc()}')
        return False


def _tg_set_bot_info():
    """Устанавливает аватар и описание бота при старте сервера."""
    if not TG_BOT_TOKEN:
        return
    try:
        # Описание бота
        req_lib.post(
            f'https://api.telegram.org/bot{TG_BOT_TOKEN}/setMyDescription',
            json={'description': 'Бот для верификации номера телефона в мессенджере WayChat. Отправьте /start для начала.'},
            timeout=5,
        )
        req_lib.post(
            f'https://api.telegram.org/bot{TG_BOT_TOKEN}/setMyShortDescription',
            json={'short_description': 'Верификация WayChat'},
            timeout=5,
        )
        app.logger.info('[TG] Бот настроен')
    except Exception as e:
        app.logger.warning(f'[TG] Ошибка настройки бота: {e}')


@app.route('/tg_webhook', methods=['POST'])
def tg_webhook():
    """Вебхук Telegram — обрабатывает сообщения боту.
    Пользователь пишет /start +79001234567 (или просто номер).
    Бот сохраняет связку phone↔chat_id.
    """
    try:
        ...
        # Команды: /start, /start 79001234567
        ...
        if cmd == '/start':
            ...
            if phone and len(phone) >= 10:
                ...
                tg = TelegramAuth.query.filter_by(phone=phone).first()
                if tg:
                    tg.chat_id    = str(chat_id)
                    tg.updated_at = datetime.utcnow()
                else:
                    tg = TelegramAuth(phone=phone, chat_id=str(chat_id))
                    db.session.add(tg)
                db_safe_commit()
                ...
                req_lib.post(
                    ...
                    json={
                        'chat_id':    chat_id,
                        'text':       f'✅ Номер *{phone}* привязан!\n\nТеперь вернитесь на сайт WayChat и нажмите «Получить код».',
                        'parse_mode': 'Markdown',
                    },
                    timeout=5,
                )
                ...
                req_lib.post(
                    ...
                    json={
                        'chat_id':    chat_id,
                        'text': (
                            '👋 Привет! Я бот для верификации WayChat.\n\n'
            ...
            # Пользователь просто прислал номер без /start
            ...
            if phone and len(phone) >= 10:
                ...
                req_lib.post(
                    ...
                    json={
                        'chat_id':    chat_id,
                        'text':       f'✅ Номер *{phone}* привязан! Вернитесь на сайт WayChat.',
                        'parse_mode': 'Markdown',
                    },
                    timeout=5,
                )
                ...
                req_lib.post(
                    ...
                    json={
                        'chat_id': chat_id,
                        'text': 'Пришлите номер телефона, например: `+79001234567`',
                        'parse_mode': 'Markdown',
                    },
                    timeout=5,
                )
        ...
        app.logger.error(f'tg_webhook: {e}')
        return jsonify({'ok': True})  # всегда 200 для Telegram


@app.route('/tg_check_phone', methods=['POST'])
@ip_rate_limit('tg_check_phone', max_calls=20, window_sec=60)
def tg_check_phone():
    """Проверяет, писал ли пользователь боту (есть ли chat_id для номера)."""
    try:
        ...
        return jsonify({
            ...
            'registered': User.query.filter_by(phone=phone).first() is not None,
            'bot_username': TG_BOT_USERNAME,
        })
    except Exception as e:
        app.logger.error(f'tg_check_phone: {e}')
...
def send_code():
    try:
        ...
        if not _tg_send_code(phone, code):
            PendingCode.delete(phone)
            return jsonify({
                'success': False,
                'error':   f'Сначала напишите боту @{TG_BOT_USERNAME} в Telegram ваш номер телефона, затем попробуйте снова.',
                'need_tg': True,
                'bot_username': TG_BOT_USERNAME,
            }), 400
...
def handle_heartbeat(data):
    """Клиент проверяет живость соединения — отвечаем ACK"""
    if not current_user.is_authenticated:
        return
    # ACK автоматически отправляется через callback если emit использует ack
    # Обновляем last_seen без коммита в БД (слишком часто)
    try:
        uid = current_user.id
        _online_cache.set(uid, True)
        # Тихо обновляем статус раз в 60 сек чтобы не перегружать БД
        now = time.monotonic()
        last_hb = _status_throttle.get(f'hb_{uid}', 0)
        if now - last_hb > 60:
            _status_throttle[f'hb_{uid}'] = now
            db.session.execute(
...
def handle_call(data):
    ...
    # Track call state for auto-miss detection
    ...
    emit('incoming_call', {
        ...
        'type':        call_type,
        'call_id':     call_id,
        'timestamp':   int(time.time()),   # клиент знает когда позвонили
    }, room=f'user_{to}')
...
def handle_answer(data):
    ...
    # Update call state → ringing accepted
    call_id = data.get('call_id', '')
    if call_id in _active_calls:
        _active_calls[call_id]['state'] = 'connected'
        _active_calls[call_id]['ts']    = time.monotonic()
...
@socketio.on('missed_call')
def handle_missed_call(data):
    """Пропущенный звонок — сохраняем в чат и уведомляем"""
    if not current_user.is_authenticated:
        return
    try:
        uid       = current_user.id
        ...
        call_type = data.get('call_type', 'audio')
        chat_id   = data.get('chat_id')
        if not to_id or not chat_id:
            return
        # Сохраняем пропущенный звонок в чат
        msg = Message(
            chat_id    = chat_id,
            sender_id  = uid,
            sender_name= uname,
            type       = f'call_{call_type}',
            content    = 'missed',
        )
        db.session.add(msg)
        db_safe_commit()
        ...
        payload = msg.to_dict()
        payload['chat_id'] = chat_id
        emit_to_user(to_id, 'new_message', payload)
        _chat_cache.delete(uid)
        _chat_cache.delete(to_id)
    except Exception as e:
        app.logger.error(f'missed_call error: {e}')


@socketio.on('call_declined')
def handle_call_declined(data):
    """Relay decline to caller — includes call_id for proper state cleanup"""
    if not current_user.is_authenticated:
        return
    ...
    emit('call_declined', {
        'from':    uid,
        'call_id': data.get('call_id', ''),
        'reason':  data.get('reason', 'declined'),  # declined|busy|timeout
    }, room=f'user_{to}')
...
if __name__ == '__main__':
    ...
    # Регистрируем вебхук и настраиваем бота при старте
    try:
        _tg_set_bot_info()
        if TG_BOT_TOKEN:
            _wh_url = f'{FRONTEND_URL}/tg_webhook'
            req_lib.post(
                f'https://api.telegram.org/bot{TG_BOT_TOKEN}/setWebhook',
                json={'url': _wh_url, 'allowed_updates': ['message']},
                timeout=10,
            )
            print(f'✅ Telegram webhook: {_wh_url}')
    except Exception as _tg_err:
        print(f'⚠️  Telegram webhook setup: {_tg_err}')

    threading.Thread(target=background_cleanup, daemon=True).start()
    _call_cleanup_task()  # auto-expire stale calls
