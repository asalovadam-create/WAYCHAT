...
(function(){
    ...
    st.textContent = `
        ...
        *, *::before, *::after {
        ...
        .msg-bubble { border-radius: 20px !important; }
        /* ── Непрочитанные цифры — БЕЛЫЕ ── */
        .wc-unread-badge { color: #fff !important; border-radius: 12px !important; }
        /* ── chat-back-badge скрыт на главном экране ── */
        #chat-back-badge:empty { display: none !important; }
        ...
        /* Scroll-to-bottom button — выше input-bar */
        #wc-scroll-btn { bottom: 84px !important; }
    ...
    // ── Avatar System CSS ──
    const _avaSt = document.createElement('style');
    _avaSt.id = 'wc-avatar-system';
    _avaSt.textContent = `
/* ═══════════════════════════════════════════════════
...
/* Размеры */
.wc-ava-chat    { width: var(--avatar-chat);    height: var(--avatar-chat);    min-width: var(--avatar-chat); }
.wc-ava-header  { width: var(--avatar-header);  height: var(--avatar-header);  min-width: var(--avatar-header); }
.wc-ava-profile { width: var(--avatar-profile); height: var(--avatar-profile); min-width: var(--avatar-profile); }
.wc-ava-msg     { width: var(--avatar-msg);     height: var(--avatar-msg);     min-width: var(--avatar-msg); }
...
/* Online dot — красивее, с мягким свечением */
.wc-ava-wrap { position: relative; flex-shrink: 0; display: inline-flex; }
.wc-ava-online {
  position: absolute; bottom: 1px; right: 1px;
  width: 11px; height: 11px;
  background: #22c55e;
  border: 2px solid var(--bg, #1d1d1e);
  border-radius: 50%;
  box-shadow: 0 0 0 1.5px rgba(34,197,94,0.35);
  animation: wcOnlinePulse 3s ease-in-out infinite;
}
@keyframes wcOnlinePulse {
  0%, 100% { box-shadow: 0 0 0 1.5px rgba(34,197,94,0.35); }
  50%       { box-shadow: 0 0 0 3px   rgba(34,197,94,0.15); }
}
/* Loading spinner для видео момента */
...
`;
    document.head.appendChild(_avaSt);
...
function insertEmoji() {
    ...
    // Переключаем на emoji-клавиатуру — меняем inputmode
    const isEmoji = inp.dataset.emojiMode === '1';
    if (isEmoji) {
        // Возврат к обычной клавиатуре
        inp.dataset.emojiMode = '0';
        inp.setAttribute('inputmode', 'text');
        inp.blur();
        requestAnimationFrame(() => inp.focus());
        const btn = document.querySelector('.tg-emoji-toggle');
        if (btn) btn.style.color = '';
    } else {
        // Открываем emoji клавиатуру
        inp.dataset.emojiMode = '1';
        // На iOS: убираем inputmode чтобы появилась нативная emoji
        inp.removeAttribute('inputmode');
        // Trigger emoji keyboard via enterkeyhint trick
        ...
        setTimeout(() => {
            ...
            inp.focus();
            // iOS Safari — симулируем tap чтобы открыть emoji
            const touch = new Touch({
                identifier: Date.now(),
                target: inp,
                clientX: inp.getBoundingClientRect().left + 10,
                clientY: inp.getBoundingClientRect().top + 10,
            });
            try {
                inp.dispatchEvent(new TouchEvent('touchstart', { touches: [touch], targetTouches: [touch], changedTouches: [touch], bubbles: true }));
                inp.dispatchEvent(new TouchEvent('touchend', { touches: [], targetTouches: [], changedTouches: [touch], bubbles: true }));
            } catch(e) {}
        }, 80);
        const btn = document.querySelector('.tg-emoji-toggle');
        if (btn) btn.style.color = 'var(--accent,#10b981)';
    }
}
...
function getAvatarHtml(user, sizeClass = 'w-12 h-12', forceRefresh = false) {
    if (!user) return `<div class="wc-ava wc-ava-chat"></div>`;
    const avatar = (!user.avatar || user.avatar === 'null' || user.avatar === 'undefined') ? '' : user.avatar;
    const name   = user.name || user.username || '?';
    const uid    = user.id || '';
    const cacheKey = `${uid}_${avatar}_${sizeClass}`;
    if (!forceRefresh && avatarHtmlCache[cacheKey]) return avatarHtmlCache[cacheKey];

    // Map sizeClass → variant class
    const sc = sizeClass || '';
    const sizeVariant = sc.includes('w-28') || sc.includes('w-20') || sc.includes('w-16') ? 'wc-ava-profile'
        : sc.includes('w-14') || sc.includes('w-12') ? 'wc-ava-chat'
        : sc.includes('w-10') || sc.includes('w-11') ? 'wc-ava-msg'
        : sc.includes('w-8')  || sc.includes('w-9')  ? 'wc-ava-header'
        : sc.includes('full') ? 'wc-ava-profile'
        : 'wc-ava-chat';

    let html;
    if (avatar.startsWith('emoji:')) {
        const emoji = avatar.split(':')[1];
        const fs = sizeVariant === 'wc-ava-profile' ? '36px'
            : sizeVariant === 'wc-ava-chat' ? '22px'
            : sizeVariant === 'wc-ava-msg'  ? '16px' : '14px';
        html = `<div class="wc-ava ${sizeVariant}" style="background:#2c2c2e;font-size:${fs}" data-uid="${uid}">${emoji}</div>`;
    } else if (avatar && !avatar.includes('default_avatar')) {
        const src = forceRefresh ? avatar + '?t=' + Date.now() : avatar;
        const fallback = getInitialAvatar(name, sizeClass, uid);
        html = `<div class="wc-ava ${sizeVariant}" data-uid="${uid}"><img src="${src}" loading="lazy" onerror="this.parentElement.outerHTML=getInitialAvatar('${name.replace(/'/g,"\\'").replace(/"/g,'&quot;')}','${sizeClass}','${uid}')"></div>`;
    } else {
...
function getInitialAvatar(name, sizeClass, uid = '') {
    // Gradient palette — 8 beautiful gradients, consistent per user
    const GRADIENTS = [
        ['#f43f5e','#e11d48'],  // rose
    ...
    const sc = sizeClass || '';
    const sizeVariant = sc.includes('w-28') || sc.includes('w-20') || sc.includes('w-16') ? 'wc-ava-profile'
        : sc.includes('w-12') || sc.includes('w-14') ? 'wc-ava-chat'
        : sc.includes('w-10') || sc.includes('w-11') ? 'wc-ava-msg'
        : sc.includes('w-8')  || sc.includes('w-9')  ? 'wc-ava-header'
        : sc.includes('full') ? 'wc-ava-profile'
        : 'wc-ava-chat';
    const gid = 'ag' + Math.abs(hash).toString(36);
    return `<div class="wc-ava ${sizeVariant}" style="background:linear-gradient(135deg,${c1},${c2})" data-uid="${uid}"><div class="wc-ava-initials">${initials}</div></div>`;
}
    ...
    <div id="chat-header" class="glass" style="padding:10px 14px;padding-top:max(env(safe-area-inset-top,0px),10px);display:flex;align-items:center;justify-content:space-between;border-bottom:0.5px solid var(--border);position:relative;z-index:5;background:var(--hdr);flex-shrink:0">
        <div style="display:flex;align-items:center;gap:10px">
            ...
            <div style="position:relative;cursor:pointer" onclick="showPartnerProfile()">
                <div id="chat-ava-header" style="width:var(--avatar-header);height:var(--avatar-header);border-radius:50%;overflow:hidden;flex-shrink:0;min-width:var(--avatar-header);"></div>
    ...
    <div class="input-bar">
        <div class="tg-input-row">
            ...
            <div class="tg-text-wrap" id="input-area">
                ...
                <button class="tg-inner-btn tg-emoji-toggle" onclick="insertEmoji()" aria-label="Эмодзи">
...
function renderChatList(chats) {
    ...
    chats.forEach((chat, index) => {
        ...
        if (isNew) {
            ...
            // Клик на аватар с моментом → открывает момент; клик на остальное → чат
            if (chat.has_moment && !isGroup) {
                avaWrap.style.cursor = 'pointer';
                avaWrap.addEventListener('click', (e) => {
                    e.stopPropagation();
                    vibrate(8);
        ...
        if (info) {
            ...
            const _TICK1 = `<svg width="14" height="10" viewBox="0 0 16 12" fill="none"><path d="M2 6l4 4L14 2" stroke="rgba(255,255,255,0.45)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
            const _TICK2 = `<svg width="18" height="10" viewBox="0 0 20 12" fill="none"><path d="M2 6l4 4L14 2" stroke="#93c5fd" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M7 6l4 4L19 2" stroke="#93c5fd" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
            // Показываем статус прочтения только для наших последних сообщений
            const _readStatusHtml = chat.last_msg_mine
                ? `<span style="display:inline-flex;align-items:center;margin-left:2px;vertical-align:middle">${chat.last_read ? _TICK2 : _TICK1}</span>`
                : '';
            info.innerHTML = `
...
async function openChat(id, name, avatar) {
    ...
    if (headerBox) {
        ...
        headerBox.innerHTML = cachedSrc
            ...
            : getAvatarHtml({id, name, avatar}, 'w-9 h-9');
...
async function openGroupChat(groupId, groupName, groupAvatar) {
    ...
    const headerBox = document.getElementById('chat-ava-header');
    if (headerBox) headerBox.innerHTML = getAvatarHtml({id:groupId, name:groupName, avatar:groupAvatar}, 'w-9 h-9');
    ...
    try {
        ...
        if (data.group_avatar && headerBox) {
            headerBox.innerHTML = getAvatarHtml({id:groupId, name:groupName, avatar:data.group_avatar}, 'w-9 h-9');
...
function buildMessageRow(msg, animate = true) {
    ...
    if (type === 'call_audio' || type === 'call_video') {
        ...
        if (_isOnce && !isMe) {
            // Получатель: заблюренное, нажать для просмотра
            contentHtml = `<div class="img-bubble" style="position:relative;max-width:220px;min-height:120px;background:rgba(255,255,255,0.06);border-radius:18px;overflow:hidden;cursor:pointer;border:1.5px solid rgba(255,255,255,0.12)"
                ...
                <img src="${_isSrc}" loading="lazy" style="display:block;width:100%;height:auto;max-height:280px;object-fit:cover;filter:blur(28px) brightness(0.4);transform:scale(1.1);pointer-events:none">
                <div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;z-index:5">
                    ...
                    <div style="font-size:10px;color:rgba(255,255,255,0.6)">🔥 Одноразовое</div>
                </div>
                <div class="msg-media-time">${displayTime}</div>
            </div>`;
        } else if (_isOnce && isMe) {
            // Отправитель: заблюрено
            contentHtml = `<div class="img-bubble" style="position:relative;max-width:220px;min-height:100px;background:rgba(239,68,68,0.08);border-radius:18px;overflow:hidden;border:1.5px solid rgba(239,68,68,0.3)">
                <img src="${_isSrc}" loading="lazy" style="display:block;width:100%;height:auto;max-height:280px;object-fit:cover;filter:blur(28px) brightness(0.4);transform:scale(1.1);pointer-events:none">
            ...
            contentHtml = `<div class="img-bubble" style="position:relative;max-width:260px;min-height:40px;background:transparent;cursor:zoom-in;border-radius:14px;overflow:hidden;border:none;outline:none;box-shadow:none"
                ...
                <img src="${_isSrc}" loading="${_isBlob?'eager':'lazy'}" decoding="async"
                    ...
                    onerror="(function(img){const sk=document.getElementById('${_skId}');if(sk)sk.remove();let r=parseInt(img.dataset.retries||0);if(r<3){img.dataset.retries=r+1;setTimeout(()=>{img.src=img.src.split('?r=')[0]+'?r='+Date.now();},1500*Math.pow(2,r));}else{img.style.display='none';img.parentElement.innerHTML='<div style=\'padding:14px 16px;color:rgba(255,255,255,.35);font-size:13px\'>⚠️ Фото не загрузилось</div>';}})(this)">
        ...
        const _thumbPlaceholder = _poster ? '' : `<div id="ph_${_vidId}" style="position:absolute;inset:0;z-index:3;background:linear-gradient(135deg,#1a1a2e,#0f3460);display:flex;align-items:center;justify-content:center;border-radius:14px"><div style="opacity:0.45"><svg width="40" height="40" viewBox="0 0 24 24" fill="none"><path d="M23 7l-7 5 7 5V7z" stroke="white" stroke-width="1.5" stroke-linecap="round"/><rect x="1" y="5" width="15" height="14" rx="2" stroke="white" stroke-width="1.5"/></svg></div></div>`;
        if (_isOnceVid && !isMe) {
            contentHtml = `<div class="video-bubble-wrap" id="${_wrpId}" onclick="openVideoModal('${_vidSrc}','${_poster||''}',true,'${msg.id}')" style="cursor:pointer;min-height:160px;max-width:220px;border:1.5px solid rgba(255,255,255,0.12)">
                <div style="position:absolute;inset:0;background:rgba(0,0,0,0.6);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;z-index:10">
                    <div style="width:52px;height:52px;border-radius:50%;background:rgba(255,255,255,0.15);backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;border:1.5px solid rgba(255,255,255,0.3)">
                        <svg width="22" height="22" viewBox="0 0 24 24" fill="white"><polygon points="5 3 19 12 5 21 5 3"/></svg>
                    </div>
                    <div style="font-size:12px;font-weight:700;color:#fff">Нажмите для просмотра</div>
                    <div style="font-size:10px;color:rgba(255,255,255,0.6)">🔥 Одноразовое видео</div>
                </div>
                <div class="msg-media-time">${displayTime}</div>
            </div>`;
        } else if (_isOnceVid && isMe) {
            contentHtml = `<div class="video-bubble-wrap" id="${_wrpId}" style="cursor:default;min-height:120px;max-width:220px;border:1.5px solid rgba(239,68,68,0.3);background:rgba(239,68,68,0.08)">
                <div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;z-index:10">
                    <div style="font-size:28px">🔥</div>
                    <div style="font-size:12px;font-weight:700;color:#fff">Одноразовое видео</div>
                    <div style="font-size:10px;color:rgba(255,255,255,0.5)">Можно посмотреть 1 раз</div>
                </div>
                <div class="msg-media-time">${displayTime}&nbsp;<span class="status-icon" style="color:${msg.is_read?'rgba(147,197,253,1)':'rgba(255,255,255,0.55)'}">${msg.is_read?ICONS.checkDouble:ICONS.check}</span></div>
            </div>`;
        ...
        contentHtml = `<div class="video-bubble-wrap" id="${_wrpId}" onclick="openVideoModal('${_vidSrc}','${_poster||''}',false,'${msg.id}')" style="cursor:pointer;min-height:160px">
            ...
            <div class="video-play-overlay"><div class="video-play-btn"><svg width="22" height="22" viewBox="0 0 24 24" fill="white"><polygon points="6 3 20 12 6 21 6 3"/></svg></div></div>
            <div class="msg-media-time">${displayTime}${isMe?`&nbsp;<span class="status-icon" style="color:${msg.is_read?'rgba(147,197,253,1)':'rgba(255,255,255,0.55)'};">${msg.is_read?ICONS.checkDouble:ICONS.check}</span>`:''}</div>
        </div>`;
        }
    ...
    // Avatars removed from message rows — messages flush to screen edge (like TG private chat)
...
function renderNewMessage(msg, animate = true) {
    ...
    // Remove any matching optimistic row before appending real message
    if (!msg._optimistic && msg.id) {
        const container = document.getElementById('messages');
        if (container) {
            container.querySelectorAll('[data-optimistic="1"]').forEach(function(el) {
                if (el.dataset.content === (msg.content || '') && !el.dataset.replaced) {
                    ...
                    return; // reuse the DOM row instead of removing
                }
            });
            // Check if we reused an optimistic row
            if (document.querySelector(`[data-msg-id="${CSS.escape(String(msg.id))}"]`)) return;
        }
    }
...
function openImgZoom(src, isOnce = false, msgId = null) {
    if (!src) return;
    document.getElementById('wc-img-viewer')?.remove();

    const viewer = document.createElement('div');
    viewer.id = 'wc-img-viewer';
    viewer.style.cssText = 'position:fixed;inset:0;z-index:99000;background:rgba(0,0,0,0.97);display:flex;align-items:center;justify-content:center;opacity:0;transition:opacity 0.22s ease';
    ...
    img.src = src;
    img.draggable = false;
    img.style.cssText = 'max-width:100vw;max-height:100vh;object-fit:contain;-webkit-user-select:none;user-select:none;-webkit-touch-callout:none';
    if (isOnce) img.style.pointerEvents = 'none';
    img.oncontextmenu = (e) => e.preventDefault();
    viewer.appendChild(img);

    // Кнопка закрыть
    const closeBtn = document.createElement('button');
    closeBtn.style.cssText = 'position:absolute;top:max(env(safe-area-inset-top,12px),12px);left:16px;width:36px;height:36px;border-radius:50%;background:rgba(255,255,255,0.12);backdrop-filter:blur(8px);border:none;color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;z-index:5;-webkit-tap-highlight-color:transparent';
    closeBtn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M18 6L6 18M6 6l12 12" stroke="white" stroke-width="2.5" stroke-linecap="round"/></svg>';
    viewer.appendChild(closeBtn);

    // Скачать — только для обычных фото
    if (!isOnce) {
        const dlBtn = document.createElement('a');
        dlBtn.href = src; dlBtn.download = 'photo.jpg';
        dlBtn.style.cssText = 'position:absolute;top:max(env(safe-area-inset-top,12px),12px);right:16px;width:36px;height:36px;border-radius:50%;background:rgba(255,255,255,0.12);backdrop-filter:blur(8px);cursor:pointer;display:flex;align-items:center;justify-content:center;z-index:5;text-decoration:none;-webkit-tap-highlight-color:transparent';
    ...
    const _doClose = () => {
        ...
        _onceCleanup();
        setTimeout(() => { viewer.remove(); document.body.style.overflow = ''; }, 220);
        if (isOnce && msgId) {
            const msgRow = document.querySelector('[data-msg-id="' + CSS.escape(String(msgId)) + '"]');
            if (msgRow) {
                msgRow.style.transition = 'opacity 0.4s,transform 0.4s';
                msgRow.style.opacity = '0';
                msgRow.style.transform = 'scale(0.9)';
                setTimeout(() => msgRow.remove(), 400);
            }
            try { socket.emit('delete_message_for_everyone', { msg_id: msgId, chat_id: currentChatId }); } catch(e) {}
            showToast('🔥 Одноразовое сообщение удалено', 'info', 2500);
        }
    };

    closeBtn.onclick = _doClose;
    viewer.addEventListener('click', (e) => { if (e.target === viewer) _doClose(); });
    document.addEventListener('keydown', function _k(e) { if (e.key === 'Escape') { _doClose(); document.removeEventListener('keydown', _k); } });

    // Свайп вниз для закрытия
    let _sy = 0;
    viewer.addEventListener('touchstart', (e) => { _sy = e.touches[0].clientY; }, { passive: true });
    viewer.addEventListener('touchend', (e) => { if (e.changedTouches[0].clientY - _sy > 80) _doClose(); });
    ...
    if (isOnce) {
        ...
        const _showGuard = () => {
            if (document.getElementById('_once_guard')) return;
            const g = document.createElement('div');
            g.id = '_once_guard';
            g.style.cssText = 'position:fixed;inset:0;z-index:9999999;background:#000;display:flex;flex-direction:column;align-items:center;justify-content:center';
            g.innerHTML = '<div style="font-size:48px;margin-bottom:16px">🚫</div><div style="font-size:17px;font-weight:700;color:#fff;margin-bottom:8px">Запись запрещена</div><div style="font-size:13px;color:rgba(255,255,255,0.5);text-align:center;padding:0 32px">Это одноразовое сообщение</div>';
            document.body.appendChild(g);
        };
        const _hideGuard = () => { document.getElementById('_once_guard')?.remove(); };

        const _visChange = () => {
            if (document.hidden) { _showGuard(); setTimeout(() => { if (document.hidden) _doClose(); }, 600); }
            else _hideGuard();
        };
        const _noCtx = (e) => e.preventDefault();

        document.addEventListener('visibilitychange', _visChange);
        window.addEventListener('blur', _showGuard);
        window.addEventListener('focus', _hideGuard);
        document.addEventListener('contextmenu', _noCtx);

        _onceCleanup = () => {
            document.removeEventListener('visibilitychange', _visChange);
            window.removeEventListener('blur', _showGuard);
            window.removeEventListener('focus', _hideGuard);
...
function toggleAudio(uid) {
    ...
    if (audio.paused) {
        ...
        // iOS earpiece routing через Web Audio API.
        // HTMLAudioElement по умолчанию → loudspeaker на iOS.
        // AudioContext + MediaElementSourceNode → earpiece (нижний динамик).
        // setSinkId не поддерживается в iOS Safari.
        try {
            if (!window._wcVoiceAudioCtx || window._wcVoiceAudioCtx.state === 'closed') {
                window._wcVoiceAudioCtx = new (window.AudioContext || window.webkitAudioContext)();
            }
            const actx = window._wcVoiceAudioCtx;
            if (actx.state === 'suspended') actx.resume().catch(() => {});
            // Создаём MediaElementSource только один раз для каждого элемента
            if (!audio._wcRouted) {
                audio._wcRouted = true;
...
function openFullImage(src) {
    ...
    overlay.innerHTML = `<img src="${src}" style="max-width:100%;max-height:100%;object-fit:contain;border-radius:4px;user-select:none">
        ...
        <a href="${src}" download style="position:absolute;bottom:max(20px,0px);left:50%;transform:translateX(-50%);background:rgba(255,255,255,0.1);border:none;color:white;font-size:13px;padding:10px 20px;border-radius:20px;text-decoration:none;display:flex;align-items:center;gap:6px"><svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><polyline points="7 10 12 15 17 10" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><line x1="12" y1="15" x2="12" y2="3" stroke="white" stroke-width="2" stroke-linecap="round"/></svg> Сохранить</a>`;
    document.body.appendChild(overlay);
}
...
function openVideoModal(src, poster) {
    ...
    loader.style.cssText = 'position:absolute;inset:0;display:flex;align-items:center;justify-content:center;z-index:5;pointer-events:none';
    loader.innerHTML = '<div class="wc-moment-spinner"></div>';
    modal.appendChild(loader);
    vid.addEventListener('canplay',  () => { loader.style.display = 'none'; },  { once: true });
    vid.addEventListener('waiting',  () => { loader.style.display = 'flex'; });
    vid.addEventListener('playing',  () => { loader.style.display = 'none'; });

    // ── Контролы ──────────────────────────────────────────
    const ctrlWrap = document.createElement('div');
    ctrlWrap.style.cssText = 'width:100%;max-width:860px;padding:14px 20px 0;display:flex;flex-direction:column;gap:10px';

    // Seek bar
    const seekBar = document.createElement('div');
    seekBar.style.cssText = 'width:100%;height:5px;border-radius:3px;background:rgba(255,255,255,0.2);position:relative;cursor:pointer;touch-action:none';
    const seekFill  = document.createElement('div');
    seekFill.style.cssText = 'height:100%;width:0%;background:var(--accent,#10b981);border-radius:3px;pointer-events:none;transition:width 0.1s linear';
    const seekThumb = document.createElement('div');
    seekThumb.style.cssText = 'position:absolute;top:50%;left:0%;width:14px;height:14px;background:#fff;border-radius:50%;transform:translate(-50%,-50%);box-shadow:0 2px 8px rgba(0,0,0,0.5);pointer-events:none';
    seekBar.appendChild(seekFill);
    seekBar.appendChild(seekThumb);

    // Кнопки
    const btnRow = document.createElement('div');
    btnRow.style.cssText = 'display:flex;align-items:center;justify-content:space-between;gap:12px';
    ...
    // Play/Pause
    const _iPlay  = `<svg width="22" height="22" viewBox="0 0 24 24" fill="none"><polygon points="5 3 19 12 5 21 5 3" fill="currentColor"/></svg>`;
    const _iPause = `<svg width="22" height="22" viewBox="0 0 24 24" fill="none"><rect x="6" y="4" width="4" height="16" rx="1" fill="currentColor"/><rect x="14" y="4" width="4" height="16" rx="1" fill="currentColor"/></svg>`;
    const btnPlay = document.createElement('button');
    btnPlay.style.cssText = 'width:54px;height:54px;border-radius:50%;background:var(--accent,#10b981);border:none;color:#000;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:transform 0.12s';
    btnPlay.innerHTML = _iPause;
    btnPlay.onclick = () => { vid.paused ? vid.play() : vid.pause(); };
    vid.addEventListener('play',  () => { btnPlay.innerHTML = _iPause; });
    vid.addEventListener('pause', () => { btnPlay.innerHTML = _iPlay; });
    vid.addEventListener('ended', () => { btnPlay.innerHTML = _iPlay; });

    // Время
    const timeLabel = document.createElement('span');
    timeLabel.style.cssText = 'font-size:12px;color:rgba(255,255,255,0.5);font-variant-numeric:tabular-nums;min-width:90px;text-align:center';
    ...
    btnSpeed.style.cssText = 'font-size:13px;font-weight:700;color:#fff;background:rgba(255,255,255,0.1);border:none;border-radius:12px;padding:8px 14px;cursor:pointer';
    btnSpeed.textContent = '1×';
    btnSpeed.onclick = () => {
        speedIdx = (speedIdx + 1) % speeds.length;
        vid.playbackRate = speeds[speedIdx];
        btnSpeed.textContent = speeds[speedIdx] === 1 ? '1×' : speeds[speedIdx] + '×';
    };

    // Сохранить — SVG иконка
    const btnSave = document.createElement('a');
    btnSave.href     = src;
    btnSave.download = '';
    btnSave.style.cssText = 'background:rgba(255,255,255,0.1);border:0.5px solid rgba(255,255,255,0.15);color:#fff;font-size:14px;padding:10px 20px;border-radius:22px;cursor:pointer;font-family:inherit;font-weight:600;text-decoration:none;display:flex;align-items:center;gap:6px;backdrop-filter:blur(8px)';
    ...
    vid.addEventListener('timeupdate', () => {
        ...
        timeLabel.textContent = `${_fmtVidTime(vid.currentTime)} / ${_fmtVidTime(vid.duration)}`;
    });
    ...
    seekBar.addEventListener('click', e => _doSeek(e.clientX));
    let _seekDrag = false;
    seekBar.addEventListener('touchstart', e => { _seekDrag = true; _doSeek(e.touches[0].clientX); }, { passive: true });
    seekBar.addEventListener('touchmove',  e => { if (_seekDrag) { e.preventDefault(); _doSeek(e.touches[0].clientX); } }, { passive: false });
    seekBar.addEventListener('touchend',   () => { _seekDrag = false; });
    ...
    let _sty = 0;
    modal.addEventListener('touchstart', e => { _sty = e.touches[0].clientY; }, { passive: true });
...
function tryBrowserNotification(msg) {
    // Только когда приложение скрыто — не показываем если пользователь в приложении
    if (!document.hidden) return;
    // Дедупликация — не показываем дважды одно сообщение (SW мог уже показать)
    if (window._lastNotifMsgId === msg.id) return;
    window._lastNotifMsgId = msg.id;
    // Очищаем через 5 сек
    setTimeout(() => { if (window._lastNotifMsgId === msg.id) window._lastNotifMsgId = null; }, 5000);
    ...
    if (Notification.permission === 'granted') {
        try {
            ...
            new Notification(senderName, {
                ...
                tag:  `wc-${msg.chat_id}`,  // tag предотвращает дубли от SW
            });
        } catch(e) {}
    }
}
...
function setupVoiceRecording() {
    ...
    if (micBtn._wcVoiceSetup) return;
    micBtn._wcVoiceSetup = true;
...
// ══ СЖАТИЕ ВИДЕО ══
async function _compressVideo(file, quality = 0.5) {
    return new Promise((resolve) => {
        try {
            const reader = new FileReader();
            reader.onload = async (e) => {
                const blob = new Blob([e.target.result], { type: 'video/mp4' });
                // Если сжатие сработало, отправляем сжатые данные
                // Если была ошибка, отправляем оригинал
...
async function pickMedia(context) {
    ...
    input.onchange = async () => {
        ...
        _showMediaPreviewModal(file);
    };
    input.click();
}

// ── Окно предпросмотра медиа ─────────────────────────────────────────
function _showMediaPreviewModal(file) {
    ...
    // Overlay
    const ov = document.createElement('div');
    ov.id = '_wc_media_preview';
    ov.style.cssText = 'position:fixed;inset:0;z-index:10000;background:rgba(0,0,0,0.96);display:flex;flex-direction:column;font-family:-apple-system,BlinkMacSystemFont,SF Pro Display,sans-serif;animation:fadeIn 0.18s ease';
    document.body.appendChild(ov);

    const close = () => { URL.revokeObjectURL(previewUrl); ov.remove(); };

    // Header
    const hdr = document.createElement('div');
    hdr.style.cssText = 'display:flex;align-items:center;justify-content:space-between;padding:max(env(safe-area-inset-top,12px),12px) 16px 12px;flex-shrink:0';
    hdr.innerHTML = `
        <button id="_pvCancelBtn" style="width:36px;height:36px;border-radius:50%;background:rgba(255,255,255,0.1);border:none;color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center">
        ...
        <span style="font-size:16px;font-weight:700;color:#fff">${isVid ? '📹 Видео' : '📷 Фото'}</span>
        <button id="_pvMenuBtn" style="width:36px;height:36px;border-radius:50%;background:rgba(255,255,255,0.1);border:none;color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center">
    ...
    hdr.querySelector('#_pvCancelBtn').onclick = close;

    // Media preview
    const mediaWrap = document.createElement('div');
    mediaWrap.style.cssText = 'flex:1;display:flex;align-items:center;justify-content:center;overflow:hidden;padding:0 8px';
    if (isVid) {
        mediaWrap.innerHTML = `<video src="${previewUrl}" style="max-width:100%;max-height:100%;border-radius:16px;object-fit:contain" controls playsinline muted></video>`;
    } else {
        mediaWrap.innerHTML = `<img src="${previewUrl}" style="max-width:100%;max-height:100%;border-radius:16px;object-fit:contain" draggable="false">`;
    }
    ...
    _updateBadge();
    ov.appendChild(modeBadge);

    // Caption + send
    const bottom = document.createElement('div');
    bottom.style.cssText = 'padding:8px 16px max(env(safe-area-inset-bottom,16px),16px);display:flex;align-items:center;gap:10px;flex-shrink:0';
    bottom.innerHTML = `
        <div style="flex:1;background:rgba(255,255,255,0.08);border-radius:24px;padding:10px 16px;display:flex;align-items:center">
            <input id="_pvCaption" placeholder="Добавить подпись..." style="flex:1;background:none;border:none;outline:none;color:#fff;font-size:15px;font-family:inherit">
        </div>
        <button id="_pvSendBtn" style="width:50px;height:50px;border-radius:50%;background:var(--accent,#10b981);border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 4px 20px rgba(16,185,129,0.4)">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M22 2L11 13M22 2l-7 20-4-9-9-4 20-7z" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
        </button>`;
    ov.appendChild(bottom);

    // 3-dot menu
    const _showMenu = () => {
        const existing = document.getElementById('_pvMenuSheet');
    ...
    bottom.querySelector('#_pvSendBtn').onclick = async () => {
        const caption = document.getElementById('_pvCaption')?.value?.trim() || '';
        close();
        await _uploadMediaWithMode(file, _sendMode, caption);
    };
}

// ── Загрузка медиа с режимом ─────────────────────────────────────────
async function _uploadMediaWithMode(file, mode, caption) {
    const isVid = file.type.startsWith('video/');
    const tmpId = 'media-upload-' + Date.now();
    let _xhrRef = null, _cancelled = false;
    ...
    tmpRow.innerHTML = `
        <div class="bubble" style="padding:4px;background:var(--accent);border-radius:18px 18px 4px 18px;position:relative;overflow:hidden;max-width:240px;min-width:120px;min-height:120px">
            ...
            <div id="${tmpId}-overlay" style="position:absolute;inset:0;background:rgba(0,0,0,0.48);display:flex;flex-direction:column;align-items:center;justify-content:center;border-radius:14px;gap:10px">
                <div style="position:relative;width:52px;height:52px">
                    <svg width="52" height="52" viewBox="0 0 52 52" style="transform:rotate(-90deg);position:absolute;inset:0">
                        ...
                        <circle id="${tmpId}-ring" cx="26" cy="26" r="20" fill="none" stroke="#fff" stroke-width="3" stroke-linecap="round"
                            stroke-dasharray="${C.toFixed(1)}" stroke-dashoffset="${C.toFixed(1)}" style="transition:stroke-dashoffset 0.3s ease"/>
                    </svg>
                ...
                <button id="${tmpId}-cancel" style="background:rgba(255,255,255,0.18);border:none;border-radius:20px;padding:5px 14px;color:#fff;font-size:12px;font-weight:600;cursor:pointer;font-family:inherit">✕ Отмена</button>
            </div>
        </div>`;

    const msgsEl = document.getElementById('messages');
    if (msgsEl) { msgsEl.appendChild(tmpRow); msgsEl.scrollTop = msgsEl.scrollHeight; }

    document.getElementById(`${tmpId}-cancel`)?.addEventListener('click', () => {
        _cancelled = true;
    ...
    if (mode === 'compressed') {
        if (!isVid && file.size > 500 * 1024) {
            try { _fileToSend = await _compressImage(file, 0.75, 1280); } catch(e) {}
        }
    ...
    // Thumbnail для видео
    let _vidThumb = null;
    if (isVid) { _extractVideoThumbnail(file).then(b => { _vidThumb = b; }).catch(()=>{}); }
    ...
    try {
        const url_res = await new Promise((resolve, reject) => {
            ...
            xhr.onload = () => {
                if (xhr.status === 413) { reject(new Error('Файл слишком большой')); return; }
            ...
            xhr.open('POST', '/upload_media'); xhr.withCredentials = true; xhr.send(fd);
        });

        if (_cancelled) return;

        // Убираем оверлей загрузки
        document.getElementById(`${tmpId}-overlay`)?.remove();
        URL.revokeObjectURL(blobUrl);
        ...
        if (isVid && _vidThumb) {
            try {
                ...
                tfd.append('file', _vidThumb, 'thumb.jpg');
                const tr = await fetch('/upload_media_thumb', { method:'POST', credentials:'include', body: tfd });
                if (tr.ok) { const tj = await tr.json(); _previewUrl = tj.url || ''; }
            } catch(e) {}
        }

        // Отправляем сообщение
        const msgType = isVid ? 'video' : 'image';
        socket.emit('send_message', {
            chat_id: currentChatId,
            type: msgType,
        ...
        if (_cancelled || e.message === 'cancelled') return;
        URL.revokeObjectURL(blobUrl);
        tmpRow.style.opacity = '0.5';
...
async function loadMoments(forceRender = false) {
    // Мгновенно рендерим из кэша — не ждём сети
    if (momentsCache) {
        _renderMomentsBar();
        _renderMomentsTab();
        // Обновляем в фоне только если кэш старше 30 сек
        if (!forceRender && Date.now() - momentsLastLoad < 30000) return;
    } else {
        // Попробуем взять из localStorage
        try {
            const lsCached = localStorage.getItem('waychat_moments_cache');
            if (lsCached) {
    ...
    try {
        ...
        if (!r.ok) { _renderMomentsBar(); _renderMomentsTab(); return; }
        ...
        try { localStorage.setItem('waychat_moments_cache', JSON.stringify(moments)); } catch(e) {}
        _renderMomentsBar();
        _renderMomentsTab();
    } catch(e) {
...
function _openMomentsOverlay(moments, startIdx) {
    ...
    function render() {
        ...
        if (isImg) {
            ...
            // ── Лоадер пока видео не готово ───────────────────────
            const loader = document.createElement('div');
            loader.style.cssText = 'position:absolute;inset:0;display:flex;align-items:center;justify-content:center;z-index:30;pointer-events:none';
            loader.innerHTML = '<div class="wc-moment-spinner"></div>';
...
function _stopAllChatVideos() {
    document.querySelectorAll('#messages video, .video-bubble-wrap video').forEach(function(v) {
        try { v.pause(); v.removeAttribute('controls'); v.muted = true; v.setAttribute('muted',''); } catch(e) {}
    });
}
...
// ══════════════════════════════════════════════════════════
//  REAL-TIME REFRESH — обновляем при возврате в приложение
// ══════════════════════════════════════════════════════════
document.addEventListener('visibilitychange', function() {
    if (document.hidden) return;
    // Приложение снова в фокусе — подгружаем свежие данные
    if (typeof loadChats === 'function') {
        setTimeout(loadChats, 400);
    }
    const chatWin = document.getElementById('chat-window');
    if (chatWin && chatWin.classList.contains('active') && typeof loadMessages === 'function') {
        setTimeout(function() { loadMessages(false); }, 400);
    }
});

// ══════════════════════════════════════════════════════════
//  SPINNER CSS для видео-лоадера в моментах и видеоплеере
// ══════════════════════════════════════════════════════════
(function() {
    var _spinSt = document.createElement('style');
    _spinSt.textContent = `
        @keyframes wcSpin { to { transform: rotate(360deg); } }
        .wc-moment-spinner {
            width: 44px; height: 44px;
